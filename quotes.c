...
int	open_double_quotes(t_env *env_var, char *input)
{
	int	i;
	int	open_quotes;
...
int	open_single_quotes(t_env *env_var, char *input)//bad name
{
	int	i;
	int	open_quotes;
...
{
	if (open_single_quotes(env_var, input)) //se voltar zero é pq ficou fechada, se não é pq ficou aberta
		return (1);
	else if (open_double_quotes(env_var, input))
		return (1);
	return (0);
}
